...
class TapHashPeginTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.tapscript_satisfy_test(CScript([OP_TAPTWEAKVERIFY, OP_1]), inputs = [valid_res, bytes.fromhex(invalid_tweaks[1]), valid_xonly_key], fail="EC scalar mult verify fail")
        self.tapscript_satisfy_test(CScript([OP_TAPTWEAKVERIFY, OP_1]), inputs = [valid_res, bytes.fromhex(invalid_tweaks[0]), valid_xonly_key], fail="Public key is neither compressed or uncompressed")

        for invalid_full_pk in invalid_full_pks:
        ...
        self.tapscript_satisfy_test(CScript([OP_TAPTWEAKVERIFY, OP_1]), inputs = [valid_res, valid_scalar, bytes.fromhex(invalid_xonly_keys[0])], fail="Public key is neither compressed or uncompressed")
        self.tapscript_satisfy_test(CScript([OP_TAPTWEAKVERIFY, OP_1]), inputs = [valid_res, valid_scalar, bytes.fromhex(invalid_xonly_keys[1])], fail="EC scalar mult verify fail")
        self.tapscript_satisfy_test(CScript([OP_TAPTWEAKVERIFY, OP_1]), inputs = [valid_res, valid_scalar, bytes.fromhex(invalid_xonly_keys[2])], fail="Public key is neither compressed or uncompressed")
        self.tapscript_satisfy_test(CScript([OP_TAPTWEAKVERIFY, OP_1]), inputs = [valid_res, valid_scalar, bytes.fromhex(invalid_xonly_keys[3])], fail="Public key is neither compressed or uncompressed")
